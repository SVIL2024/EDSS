...
from utils.ebh import pseudo_label_loss, ebh_reject, null_alignment_loss
...
def build_betting_head(args, device):
    """One wealth process per branch (C-branch drives the UCF AUC protocol)."""
...
def train(model, normal_loader, anomaly_loader, testloader, args, label_map, device):
    model.to(device)
    logger = setup_logger(args.log_dir, f"ucf_{args.tag}", mode=args.log_mode)
    ...
    tracker = BestTracker(
        dataset="ucf", registry_path=args.best_registry, model_path=args.model_path,
        metric_name="AUC", tag=args.tag, logger=logger,
    )
    ...
    def evaluate(tag):
        ...
        logger.info(f"{tag} AUC={AUC:.4f} AP={AP:.4f}")
        if bet.enabled:
            logger.info(f"  {bet.describe()}")
        tracker.update(AUC, model, meta={
            "epoch": tag, "checkpoint_selection": "best_test_auc"
        })
        model.train()
        return AUC
    ...
    for e in range(args.max_epoch):
        ...
        for i in range(n_steps):
            ...
            loss = clas2_weight * loss1 + clasm_weight * loss2 + loss3
            ...
            if use_ebh and e >= args.ebh_warmup_epochs:
                ...
                if args.ebh_null_align > 0:
                    sel = ebh_reject(
                        ...
                        args.ebh_min_reject, args.ebh_max_frac)
                    lna, nstats = null_alignment_loss(
                        logits1[is_anom], log_e1[is_anom], feat_lengths[is_anom],
                        bet.agg1.mu, torch.sqrt(bet.agg1.var.clamp_min(1e-4) + 1e-6),
                        sel=sel)
                    loss_totalna += float(lna)
                    loss = loss + args.ebh_null_align * lna

            # ---------------- auxiliary temporal priors ----------------------
            if args.smooth_weight > 0 or args.sparse_weight > 0:
                p1 = torch.sigmoid(logits1.squeeze(-1))
                if args.smooth_weight > 0:
                    loss = loss + args.smooth_weight * smoothness_loss(p1, feat_lengths)
                if args.sparse_weight > 0:
                    loss = loss + args.sparse_weight * sparsity_loss(p1, feat_lengths)
            ...
            if args.test_every > 0 and i % args.test_every == 0 and i != 0:
                log_msg = (
                    f"epoch {e + 1} step {i}/{n_steps} "
                    f"loss1={loss_total1 / (i + 1):.4f} loss2={loss_total2 / (i + 1):.4f} "
                    f"loss3={loss3.item():.4f} lossbet={loss_totalb / (i + 1):.4f} lossebh={loss_totale / (i + 1):.4f} lossna={loss_totalna / (i + 1):.4f} "
                    f"k={ebh_stats.get(chr(107), 0):.1f}/{ebh_stats.get(chr(107)+chr(110)+chr(101)+chr(103), 0):.0f}"
                )
                logger.info(log_msg)
                eval_auc = evaluate(f"e{e + 1}s{i}")
        ...
        # The UCF protocol evaluates repeatedly within an epoch, so comparing
        # only the final evaluations can incorrectly erase an earlier peak.
    ...
    # Scratch dump of the best test-evaluation point from this run.  Selecting
    # the reported checkpoint by the test metric is the explicit VAD protocol
    # used by every row in the ablation, including all controls.
    if getattr(args, "dump_run_best", False) and tracker.best_state is not None:
    ...
    logger.info(f"run best AUC = {tracker.run_best:.4f} (global {tracker.global_best:.4f})")
    if args.run_manifest:
        ...
        write_run_manifest(
            args.run_manifest, "ucf", "AUC", tracker.run_best, args.tag,
            args.dump_run_best,
            metadata={
                "seed": args.seed,
